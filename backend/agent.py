...
SYSTEM_PROMPT = """You are LinguaFlow, a warm, encouraging, and expert English language tutor. Your mission is to help users improve their spoken English through conversation practice, grammar correction, vocabulary building, and real-world scenario practice.
...
class LanguageTutorAgent:
    ...
    async def process_message(
        ...
        # Compose the user message with context
        ...
        try:
            ...
            if tool_results:
                # If tools were detected, send tool results back for final response
    ...
    def _detect_and_execute_tools(self, response: str, user_text: str) -> list:
        """Detect if tools should be used based on user intent and execute them."""
        results = []
        ...
        if any(phrase in text_lower for phrase in [
            ...
            "vocabulary", "what means"
        ]):
            # Extract the word (simple heuristic)
            words = user_text.split()
            target_word = words[-1] if len(words) > 2 else user_text
            for w in words:
                if w.lower() not in ["what", "does", "mean", "meaning", "of", "the", "is", "a", "define", "word"]:
                    target_word = w
                    break
        ...
        if any(phrase in text_lower for phrase in [
            ...
            if "interview" in text_lower or "job" in text_lower:
            ...
            elif "travel" in text_lower or "airport" in text_lower or "hotel" in text_lower:
            ...
            elif "shop" in text_lower or "buy" in text_lower:
                scenario_type = "shopping"
            elif "doctor" in text_lower or "health" in text_lower:
